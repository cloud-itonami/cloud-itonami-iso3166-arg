  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems AR"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems AR" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest ar-entity-missing-is-held-and-unoverridable
  (testing "missing Japan-resident authorized representative -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:ar-entity-missing} (-> (store/ledger db) last :basis)))
...
(deftest cuit-unverified-is-held-and-unoverridable
  (testing "unverified corporate number when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:cuit-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
