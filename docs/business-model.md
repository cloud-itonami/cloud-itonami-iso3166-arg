# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Argentina

## Classification

- Repository: `cloud-itonami-iso3166-arg`
- ISO 3166: `ARG` (Argentina)
...
- Social impact: [:compre-argentino-preference-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on an Argentine
  public contract
...
- registration walkthrough for COMPR.AR, Argentina's official public procurement portal
- business/tax registration checklist: a CUIT (Clave Única de Identificación Tributaria) tax registration via ARCA (formerly AFIP); VAT (IVA) registration with ARCA
- local-content / preferential-procurement navigation: the Compre Argentino y Contrate Nacional program's preference margins for domestically-manufactured goods and local labor on qualifying public tenders
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Argentine-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Argentina).
