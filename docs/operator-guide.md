...
2. Register the client's intake: business type, target public function,
   prior filing history in Argentina if any.
3. Run the advisor in read-only mode against COMPR.AR, Argentina's official public procurement portal.
...
- named referral relationship with Argentine-licensed counsel or a
...
a working referral relationship with Argentine-licensed counsel or a
registered agent for whatever licensed representation the law of
Argentina requires for actual public-procurement filings.
