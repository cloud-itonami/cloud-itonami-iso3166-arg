(ns marketentry.marketentryllm
  ...
  the ARG public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-ar-entity?=" (:has-ar-entity? e)
                        " cuit-verified?=" (:cuit-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-ar-entity? e))
                              (:has-ar-entity? e))
                          (or (not (:requires-cuit? e))
                              (:cuit-verified? e)))
                   0.9 0.3)}))
