(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether an Argentina-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Argentina-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-ar-entity?
                                       true` (near-universal for Argentina
                                       ...
                                       Grounded in Argentine legal entity
                                       (CUIT) typically required for federal
                                       awards (contracting authorities / ONC).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(CUIT/統一資格/COMPR.AR登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- ar-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ar-entity? true`, INDEPENDENTLY verify
  `:has-ar-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-ar-entity?` ground truth (most Argentina public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ar-entity? e))
                 (not (true? (:has-ar-entity? e))))
        [{:rule :ar-entity-missing
          :detail (str subject " はアルゼンチン居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- cuit-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-cuit? true`, INDEPENDENTLY check
  `:cuit-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-cuit? e))
                 (not (true? (:cuit-verified? e))))
        [{:rule :cuit-unverified
          :detail (str subject " はCUIT確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (ar-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (cuit-unverified-violations request st)
