(ns marketentry.store
  "SSoT for the ARG market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems AR" :portal "COMPR.AR"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ar-entity? true :has-ar-entity? true
             :requires-cuit? true :cuit-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ARG" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "COMPR.AR"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ar-entity? true :has-ar-entity? true
             :requires-cuit? true :cuit-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "COMPR.AR"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "ARG" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "COMPR.AR"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ar-entity? true :has-ar-entity? false
             :requires-cuit? true :cuit-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ARG" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "COMPR.AR"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ar-entity? true :has-ar-entity? true
             :requires-cuit? true :cuit-verified? false
             :drafted? false :submitted? false
             :jurisdiction "ARG" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "COMPR.AR"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-ar-entity? has-ar-entity?
                               requires-cuit? cuit-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-ar-entity?)  (assoc :engagement/requires-ar-entity? requires-ar-entity?)
    (some? has-ar-entity?)       (assoc :engagement/has-ar-entity? has-ar-entity?)
    (some? requires-cuit?)    (assoc :engagement/requires-cuit? requires-cuit?)
    (some? cuit-verified?)    (assoc :engagement/cuit-verified? cuit-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-ar-entity? :engagement/has-ar-entity?
   :engagement/requires-cuit? :engagement/cuit-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-ar-entity? (boolean (:engagement/requires-ar-entity? m))
     :has-ar-entity? (boolean (:engagement/has-ar-entity? m))
     :requires-cuit? (boolean (:engagement/requires-cuit? m))
     :cuit-verified? (boolean (:engagement/cuit-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-arg"})
