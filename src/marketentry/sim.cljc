...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (ARG, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up ar-entity-missing) ==")
    ...
    (println "== filing/submit eng-4 (ar-entity-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up cuit-unverified) ==")
    ...
    (println "== filing/submit eng-5 (cuit-unverified -> HARD hold) ==")
